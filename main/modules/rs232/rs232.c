...
static dataset_t ds_ac_out_power;
static dataset_t ds_pv_power;
static inv_info_t inv_info = {
    0,
};
...
void inv_store_energy_meas() {
  ...
  if (energy_history.size == 0) {
    ...
    if (energy_meas.grid_consumed < min_energy_diff &&
        energy_meas.ac_output < min_energy_diff &&
        energy_meas.pv < min_energy_diff &&
        energy_meas.battery_charge < min_energy_diff &&
        energy_meas.battery_discharge < min_energy_diff) {
      /*struct tm timeinfo;
  ...
  ESP_LOGI(LOG_TAG, "Total grid energy consumed: %.2fkWh",
           energy_meas.grid_consumed / 3600.0 / 1000);
  ESP_LOGI(LOG_TAG, "Total AC out energy: %.2fkWh",
           energy_meas.ac_output / 3600.0 / 1000);
  ESP_LOGI(LOG_TAG, "Total PV energy: %.2fkWh", energy_meas.pv / 3600.0 / 1000);
  ESP_LOGI(LOG_TAG, "Total battery charge: %.2fkWh",
           energy_meas.battery_charge / 3600.0 / 1000);
  ESP_LOGI(LOG_TAG, "Total battery discharge: %.2fkWh",
           energy_meas.battery_discharge / 3600.0 / 1000);

  energy_meas.grid_consumed = 0;
  energy_meas.grid_produced = 0;
  energy_meas.ac_output = 0;
  energy_meas.pv = 0;
  energy_meas.battery_charge = 0;
  energy_meas.battery_discharge = 0;
...
void inv_set_meas_ac_out(uint16_t voltage, uint16_t power, uint16_t freq,
  ...
  if (ac_output_meas.timestamp != 0) {
    ...
    energy_meas.ac_output += delta_energy;
